...
public class JumperGame {
	...
	public int longestJump(int[][] mat, int i, int j, int oppennent) {
		int N = mat.length;
		if (!firstStart)
			if (i >= N || i < 0 || j >= N || j < 0 || mat[i][j] != 0 || visited[i][j])
				return 0;
		int length = firstStart ? -1 : 0;
		visited[i][j] = true;
		firstStart = false;
		int longest = 0;
		if (i - 1 >= 0 && mat[i - 1][j] == oppennent)
			longest = max(longest, longestJump(mat, i - 2, j, oppennent));
		if (i + 1 < N && mat[i + 1][j] == oppennent)
			longest = max(longest, longestJump(mat, i + 2, j, oppennent));
		if (j - 1 >= 0 && mat[i][j - 1] == oppennent)
			longest = max(longest, longestJump(mat, i, j - 2, oppennent));
		if (j + 1 < N && mat[i][j + 1] == oppennent)
			longest = max(longest, longestJump(mat, i, j + 2, oppennent));
		visited[i][j] = false;
		return length + longest + 1;
	}
	...
	public void test1() {
		...
		board[2] = new int[] { 0, 2, 1, 0, 0 };
		board[3] = new int[] { 0, 0, 0, 0, 0 };
		board[4] = new int[] { 0, 0, 0, 0, 0 };

		displayBoard(board);
		int length = o.longestJump(board, 2, 2, 2);
