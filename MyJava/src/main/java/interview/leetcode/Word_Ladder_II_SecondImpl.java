...
public class Word_Ladder_II_SecondImpl {

	public static void main(String[] args) {
		Word_Ladder_II_SecondImpl obj = new Word_Ladder_II_SecondImpl();

		String start0 = "a";
		String end0 = "c";
		Set<String> dict0 = new HashSet<String>();
		dict0.add("a");
		dict0.add("b");
		dict0.add("c");
		List<List<String>> res0 = obj.findLadders(start0, end0, dict0);
		...
		dict01.add("hot");
		dict01.add("dog");
		List<List<String>> res01 = obj.findLadders(start01, end01, dict01);
		...
		List<List<String>> res2 = obj.findLadders(start2, end2, dictraw);
		long finish = System.currentTimeMillis();
		System.out.println("res2 solution size:" + res2.size() + "\t time: "
				+ (finish - begin));

		String start22 = "sand";
		String end22 = "acne";
		String[] raw22 = { "slit", "bunk", "wars", "ping", "viva", "wynn",
		...
		dictraw = new HashSet<String>();
		for (String str : raw22)
			dictraw.add(str);
		begin = System.currentTimeMillis();
		List<List<String>> res22 = obj.findLadders(start22, end22, dictraw);
		finish = System.currentTimeMillis();
		System.out.println("res22 solution size:" + res22.size() + "\t time: "
				+ (finish - begin));
		...
		for (String str : raw22)
			dictraw.add(str);
		begin = System.currentTimeMillis();
		Word_Ladder_II obj2 = new Word_Ladder_II();
		List<List<String>> res222 = obj2.findLadders_Improved2(start22, end22, dictraw);
		finish = System.currentTimeMillis();
		System.out.println("res22 optimal size:" + res222.size() + "\t time: "
				+ (finish - begin));
	}
	...
	public List<List<String>> findLadders(String start, String end,
		...
		Map<String, List<String>> precursors = new HashMap<String, List<String>>();
		...
		while (!q.isEmpty()) {
			...
			for (int i = 0; i < levelSize; i++) {
				...
				for (int j = 0; j < wchs.length; j++) {
					char original = wchs[j];
					for (char c = 'a'; c <= 'z'; c++) {
						if (c == original)
							continue;
						wchs[j] = c;
						String dist1word = new String(wchs);
						if (dict.contains(dist1word)) {// find path, connected
							if (visited.add(dist1word))
								q.add(dist1word);
							if (!precursors.containsKey(dist1word)) {
								precursors.put(dist1word, new ArrayList<String>());
							} // add successor
							precursors.get(dist1word).add(w);
		...
		path.push(start);
		buildPath(res, start, end, length - 1, path, precursors);
		return res;
	}

	private void buildPath(List<List<String>> paths, String start, String end,
			int depth, Stack<String> path, Map<String, List<String>> successors) {
		if (depth == 0) {
			if (start.equals(end)) {
		...
		List<String> neighbors = successors.get(start);
		if (neighbors == null)
			return;
		for (String succ : neighbors) {
			path.push(succ);
			buildPath(paths, succ, end, depth - 1, path, successors);
			path.pop();
		}
	}
