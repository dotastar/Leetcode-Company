...
public class JobCenter {
  @Inject
  private CrawlByCity crawlByCity;
  ...
  public void parseAllRawPageTask() throws IOException {
    ...
    while (rawPageDBCursor.hasNext()) {
      ...
      if (dealerPosts.isEmpty()) {
        log.error("Unexpected error: page {} has no results(DealerPost).", rawPage);
        continue;
      }
  ...
  private Map<String, City> buildCityMap() throws IOException {
    ...
    return cityList.stream().collect(
        Collectors.toMap(c -> DealerListPageParser.extractCityNameFromUrl(c.getUrl()), Function.identity()));
  }
